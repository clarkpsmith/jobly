...
describe("GET /companies", function () {
  ...
  test("test get companies with one query params", async function () {
  ...
  test("test get companies with multiple query params ", async function () {
    const resp = await request(app).get("/companies?name=c3&minEmployees=2");
    expect(resp.body).toEqual({
      companies: [
        {
          handle: "c3",
  ...
  test("throws 400 error if minEmployees value is greater than maxEmployees value", async function () {
    const resp = await request(app).get(
      "/companies?maxEmployees=1&minEmployees=2"
    );
    expect(resp.statusCode).toEqual(400);
  });
